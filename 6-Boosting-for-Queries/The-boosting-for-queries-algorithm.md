...
**隐私** 我们将证明整个序列$$(S_1,\mathcal{A}_1,...,S_T,\mathcal{A}_T)$$可以在保留差分隐私的同时输出。请注意，这比我们需要的要强——我们实际上并不输出集合$$S_1,...,S_T$$。根据我们的自适应组合定理，每个$$\mathcal{A}_i$$的隐私将由基本概要生成器的隐私保证以及$$S_{i-1}$$以差分隐私方式计算的事实来保证。因此，只要证明$$(S_1,\mathcal{A}_1,...,S_i,\mathcal{A}_i)$$是差分私有的，$$S_{i+1}$$也是差分私有的。然后，我们可以使用组合定理组合隐私参数来计算最终保证。

**引理6.6.** 使$$\epsilon^*=\frac{4\alpha T\rho}{\mu}$$.对所有$$i \in [T]$$，一旦$$(S_1,\mathcal{A}_1,...,S_i,\mathcal{A}_i)$$被固定，$$S_{i+1}$$的每个元素的计算是$$(\epsilon^*,0)$$-差分私有的。

【证明】固定$$\mathcal{A}_1,...,\mathcal{A}_i$$，对于每个$$j \leq i$$，数量$$d_{q,j}$$具有灵敏度$$\rho$$，因为$$\mathcal{A}_j(q)$$与数据库无关（因为$$\mathcal{A}_j$$是固定的），并且每个$$q \in \mathcal{Q}$$的敏感度受$$\rho$$限制。因此，对于每个$$j \leq i$$，$$a_{j,q}$$通过构造对$$2\rho/\mu$$敏感，所以
$$
\begin{aligned}
g_i(q)\overset{def}{=}\sum_{j=1}^{i}a_{j,q}
\end{aligned}
$$

灵敏度至多为$$2i\rho/\mu \leq 2T\rho/\mu$$。那么$$\Delta g_i\overset{def}{=}2T\rho/\mu$$是$$g_i$$灵敏度的上限。为了探讨隐私，我们将证明$$S_{i+1}$$的查询选择是指数机制的一个实例。将$$−g_i(q)$$视为第$$i + 1$$轮选择过程中查询$$q$$的效用。指数机制表示，为了实现 $$(\epsilon^*, 0)$$-差分隐私，我们应该选择$$q$$以概率正比于
$$
\begin{aligned}
exp(-g_i(q)\frac{\epsilon^*}{2\Delta g_i}).
\end{aligned}
$$

由于$$\epsilon^*/2\Delta g_i=\alpha$$并且算法以与$$e^{−\alpha g_i(q)}$$成比例的概率选择$$q$$，我们看到这正是算法所做的！

我们通过将查询的每个选择视为“迷你机制”来限制释放$$S_i$$s的隐私损失，在$$T$$轮提升过程中，该机制被调用$$kT$$次。根据引理6.4，每个迷你机制都是$$(4\alpha T\rho/\mu,0)$$-差分私有的。根据定理3.20，对于所有$$\beta>0$$，$$kT$$机制的组成（每个机制都是$$(4\alpha T\rho/\mu,0)$$-差分私有的）是$$(\epsilon_{sample},\delta_{sample})$$-差分私有的，其中
$$
\begin{aligned}
\epsilon_{sample} \overset{def}{=} \sqrt{2kTlog(1/\delta_{sample})}(\alpha 4T\rho/\mu)+kT(\frac{\alpha 4T\rho}{\mu})^2. \qquad \qquad (6.7)
\end{aligned}
$$
