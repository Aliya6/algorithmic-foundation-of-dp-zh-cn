...
我们通过将查询的每个选择视为“迷你机制”来限制释放 Sis 的隐私损失，在 T 轮提升过程中，该机制被调用 kT 次。根据引理 6.4，每个微型机制都是 (4αT ρ/μ, 0) 差分私有的。根据定理 3.20，对于所有 β > 0，kT 机制的组成（每个机制都是 (α4T ρ/μ, 0) 差分隐私）是 (εsample, δsample)-差分隐私，其中

我们总的隐私损失来自于对基本消毒剂的 T 次调用的组成以及 kT 个样本的累积损失。我们得出结论，整个 boosting 算法是： (εboost, δboost) 差分私有的，其中
