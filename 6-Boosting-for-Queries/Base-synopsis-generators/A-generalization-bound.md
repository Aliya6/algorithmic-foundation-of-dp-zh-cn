...
【引理 6.5 的证明】 根据$$\mathcal{D}^{a \cdot m}$$独立选择的一组固定查询$$S \subset \mathcal{Q}$$。检查任意$$m$$位概要$$y$$。请注意，$$y$$由$$m$$位字符串描述。假设$$|\mathcal{R}(y, q) − q(x)|＞\lambda$$对$$\mathcal{D}$$的至少$$(log(1/\gamma)+m)/(a \cdot m)$$成立则 $$y$$不好，这意味着$$Pr_{q∼\mathcal{D}}[|\mathcal{R}(y, q) − q(x)|＞ \lambda] \geq (log(1/\gamma) + m)/(a \cdot m)$$.

换句话说，如果存在分数权重至少为$$(log(1/\gamma) + m)/(a \cdot m)$$的集合$$\mathcal{Q}_y ⊂ Q，使得 |R(y, q) − q(x)，则 y 是坏的| > λ 对于 q ∈ Qy。对于这样的 y，y 对每个 q ∈ S 给出 λ 精确答案的概率是多少？这正是没有任何一个的概率
